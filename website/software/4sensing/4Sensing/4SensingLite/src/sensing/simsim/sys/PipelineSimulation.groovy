...
import sensing.core.vtable.VTableManager
import simsim.core.*
...
public abstract class PipelineSimulation extends Simulation implements Displayable {
	static ServicesConfig.QueryImplPolicy QUERY_IMPL_POLICY;
	
	static BasicCentralizedSetup setup;
	static String runId;
	static String simId;
	static String env;
	static boolean exit;
...
//	}
	...
	protected void init() {
		...
		mNodeDB = new MobileNodeDB();
		Globals.set("4sensing_MobileNodeDB", mNodeDB);
		...
		Gui.addInputHandler("MainFrame", [onMouseMove: { XY pu, XY ps ->}, onMouseDragged: { int button, XY pu, XY ps -> },
		    onMouseClick: { int button, XY pu, XY ps -> 
				if(button == 1) {
					//closestNode(ps, onMouseClick); //smd
...
//		};
		...
		new PeriodicTask(60) {
			public void run() {
				try {
					simsim.gui.pdf.PDF.saveToPDF( PipelineSimulation.this, "/tmp/foo.pdf", 1000, 1000) ;
				} catch( Exception x ){}
			}
		};
