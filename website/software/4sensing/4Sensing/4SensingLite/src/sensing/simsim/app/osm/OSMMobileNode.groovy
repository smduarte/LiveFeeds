...
public class OSMMobileNode extends MobileNode {
	...
	public void reportPosition() {
		...
		if(homeBase.isOnline() ) {
			//homeBase.services.logging.log(DEBUG, this, "reportPosition", "sending reading");
			...
			boolean accepted = sensorInput(r);
			if(accepted) {
				//homeBase.services.logging.log(DEBUG, this, "reportPosition", "reading accepted");
				...
				//homeBase.services.logging.log(DEBUG, this, "reportPosition", "reading not accepted");
			}		
		} else {
			homeBase.services.logging.log(DEBUG, this, "reportPosition", "homebase is offline");
	...
	public void displayOn(Canvas c) {
		if(cPos) {
			...
			if(speedSenseModel.isCongested(cSegmentId, cSpeed)) {
				...
				statusColor = RGB.GREEN.darker();
			}
			XY cPosXY = getPos();
			if(homeBase.debugNode) {
				c.sFill( statusColor, new Circle(cPosXY.x, cPosXY.y, 5));
			} else {
				c.sFill( statusColor, new Rectangle(cPosXY.x, cPosXY.y, 2.0, 2.0));
			}
		}
