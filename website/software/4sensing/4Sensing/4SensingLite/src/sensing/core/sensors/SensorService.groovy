...
import sensing.core.network.UnavailablePeer;
import sensing.core.pipeline.Tuple;
import sensing.core.sensors.monitoring.WorkloadSensor;
...
//			}
			...
			if(b && availableDest.indexOf(b.dest) != -1) { // keep current destination
				services.logging.log(DEBUG, this, "send", "rebinding - keep current destination"); //smd
				b.bounds = bounds;
			} else {
				services.logging.log(DEBUG, this, "send", "rebinding - new destination"); //smd
				b = new Binding();
				b.dest = availableDest[services.random.nextInt(availableDest.size())];
				b.bounds =  bounds;
			}
		}
		//services.logging.log(DEBUG, this, "send", "sending tuple to ${b.dest.id}"); //smd
		try{
			boolean accepted = b.dest.services.sensor.input(services.network.local,  reading);
			if(accepted) {
				//services.logging.log(DEBUG, this, "send", "sending tuple - accepted"); //smd
	...
	protected boolean input(Peer src, reading) {
		...
		//services.logging.log(DEBUG, this, "input", "${reading.class.name} reading accepted: $accepted")
		return accepted;
	}
