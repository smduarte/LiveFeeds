...
//	protected OSMMapModel mapModel = SpeedSenseSim.mapModel;
...
//	public double getSpeed(segment, Node node) {
//		def wayNodes = segment.way.nodes;
//		int startIdx = wayNodes.indexOf(segment.start);
//		int endIdx = wayNodes.indexOf(segment.end);
//		def fwdNode;
//		def bckNode;
//		if(endIdx > startIdx) {
//			def fwdInter = mapModel.getWayIntersections(segment.way, segment.start, true, false);
//			node.services.logging.log(DEBUG, this, "getSpeed", "forward: ${fwdInter}");
//			fwdNode = fwdInter ?  fwdInter[0][0] : wayNodes.last();
//			def bckInter = mapModel.getWayIntersections(segment.way, segment.end, false, false);
//			node.services.logging.log(DEBUG, this, "getSpeed", "back: ${bckInter}");
//			bckNode = bckInter ? bckInter[0][0] : wayNodes.first();
//		} else {
//			def fwdInter = mapModel.getWayIntersections(segment.way, segment.start, false, false);
//			fwdNode = fwdInter ?  fwdInter[0][0] : wayNodes.first();
//			def bckInter = mapModel.getWayIntersections(segment.way, segment.end, true, false);
//			bckNode = bckInter ? bckInter[0][0] : wayNodes.last();
//		}
//		assert(fwdNode && bckNode);
//		String segmentId = "${bckNode.id}_${fwdNode.id}";
...
//	}
	...
	public double getSpeed(segment, String segmentId, MobileNode node) {
		...
		//String segmentId = "${segment.way.id}_${segmentN}";
		getSpeedImpl(segment, segmentId, node);
	}
	...
	protected double getSpeedImpl(segment, segmentId, MobileNode node) {
		...
		//node.homeBase.services.logging.log(DEBUG, this, "getSpeed", "segment: ${segmentId} density:${segmentNodeDensity[segmentId]} avgSpeed:${avgSpeed} speed:${speed} type: ${segment.way.keys['highway']}");
		return speed;
	}
