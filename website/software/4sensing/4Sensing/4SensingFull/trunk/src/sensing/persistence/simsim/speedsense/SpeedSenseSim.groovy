...
public abstract class SpeedSenseSim extends PipelineSimulation implements Displayable {
	public static MapView  mapView;
	public static MapModel mapModel;
	Rectangle2D.Double screenBounds;

	protected SpeedSenseSim() {
		super();
		...
		display.queryError = true;
		display.mobile = true;
		display.segmentExtent = false;
		display.state = true;
		display.stateFilter = [];
		display.grid = 0;
		display.workLoad = false;
	}
...
//		PeerDB.peerFilterWidth  = delta[1];
	...
	public void center(SpeedSenseNode n) {
	...
	protected  sensing.persistence.simsim.Node getClosestNode(XY pos) {
		def closest;
