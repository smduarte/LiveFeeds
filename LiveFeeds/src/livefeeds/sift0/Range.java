...
import simsim.utils.RandomList2;

public class Range {
	protected static final long KEY_RANGE = (1L << Config.NODE_KEY_LENGTH);
	protected static final long MAX_KEY = KEY_RANGE - 1L;
...
//	}
	...
	public RandomList2<CatadupaNode> nodeList(DB db ) {
		RandomList2<CatadupaNode> res = new RandomList2<CatadupaNode>() ;
	...
	public RandomList2<CatadupaNode> nodeList(DB db, Event e ) {
		RandomList2<CatadupaNode> res = new RandomList2<CatadupaNode>() ;
		for( CatadupaNode i : nodeList(db) ) {
			e.evaluatedBy( db.owner, i ) ;
			if( i.accepts(e) )
				res.add(i) ;
		}
		return res ;
	...
	public List<Range> slice(int level, int slices, DB db) {
		...
		return level < 0.15 * h ? sliceKeys( slices) : sliceNodes(slices, db);
	}

	protected static int height(int G, int n) {
		n = n + 1;
		double L = (G - 1) * n + 1;
		return (int) Math.floor(Math.log(L) / Math.log(G) - 1);
	}

	protected int[] children2(int G, final int N) {
		if (G == 1)
			return new int[] { N };
		else {
			int[] res = new int[G];
	...
	protected int[] children(int G, final int N) {
