...
public class CatadupaTraffic implements Cloneable {
	
	LinkedList<CatadupaTraffic> history = new LinkedList<CatadupaTraffic>() ;
	
	public CatadupaTraffic() {
		updateHistory() ;
	}
	...
	public double sliceCasting_upload = 0, sliceCasting_download = 0;
	...
	public double casting_upload() {
		return catadupaCasting_upload ;		
	}

	public double casting_download() {
		return catadupaCasting_download ;
	}

	public double slicecasting_upload() {
		return joinRequest_upload + sliceCasting_upload ;		
	}

	public double slicecasting_download() {
		return joinRequest_download + sliceCasting_download ;
	}
	...
	public double upload() {
		return repair_upload() + db_upload() + casting_upload() + departure_upload() + slicecasting_upload() ;
	}
	
	public double download() {
		return repair_download() + db_download() + casting_download() + departure_download() + slicecasting_download();
	}
	
	public double recent_upload_rate() {
		
		CatadupaTraffic other = history.getLast() ;
	...
	public double recent_slicecasting_upload_rate() {
		
		CatadupaTraffic other = history.getLast() ;
		
		double elapsed = Simulation.currentTime() - other.timeStamp ;
		double total = this.slicecasting_upload() - other.slicecasting_upload() ;
		
		return total / elapsed ;
	}
	
	public double recent_slicecasting_download_rate() {
		CatadupaTraffic other = history.getLast() ;
		
		double elapsed = Simulation.currentTime() - other.timeStamp ;
		double total = this.slicecasting_download() - other.slicecasting_download() ;
	...
	public CatadupaTraffic clone() {
		try {
			timeStamp = Simulation.currentTime() ;			
			return (CatadupaTraffic)super.clone() ;
