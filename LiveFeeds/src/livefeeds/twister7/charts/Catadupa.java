package livefeeds.twister7.charts;
...
//			late_average_rates();
//
			detailed_upload_download_rates_15min() ;
//			detailed_download_rates() ;
//			sessionRatiosDeviation() ;
			total_averages();
//			detailed_rates15min();
//			total_averages() ;
	...
	String cat_filename(String f) {
		return "/Users/smd/runs/" + CAT_PREFIX + f + "-stats.xml";
	}
...
//		new DoubleVerticalChart("Cat_Sessions", "Catadupa Upload / Download Ratios", "entire session","last 15 minutes", mcu, mcd).saveChart("/tmp/charts/cat_50k_session_ratios.pdf") ;		
	...
	private void _15min_average_rates() throws Exception {
	...
	private void detailed_upload_download_rates_15min() throws Exception {
		Statistics S0 = Persistent.loadFromXml(cat_filename("A4_M8_250_00_38"));
		
		List<BinnedTally> a = new ArrayList<BinnedTally>();
		add(a, S0.catadupaRecentAvgTraffic.catDaparture_d,"departures");
		add(a, S0.catadupaRecentAvgTraffic.dbRepair_d,"repair");
		add(a, S0.catadupaRecentAvgTraffic.dbEndpoints_d,"endpoints");
		add(a, S0.catadupaRecentAvgTraffic.dbFilters_d, "filters");
		add(a, S0.catadupaRecentAvgTraffic.casting_d, "broadcasting");

		List<BinnedTally> b = new ArrayList<BinnedTally>();
		add(b, S0.catadupaRecentAvgTraffic.catDaparture_u,"departures");
		add(b, S0.catadupaRecentAvgTraffic.dbRepair_u,"repair");
		add(b, S0.catadupaRecentAvgTraffic.dbEndpoints_u,"endpoints");
		add(b, S0.catadupaRecentAvgTraffic.dbFilters_u, "filters");
		add(b, S0.catadupaRecentAvgTraffic.casting_u, "broadcasting");

		DeviationBinnedTallyDisplay mcu = new StackedBinnedTallyDisplay(a);
...
//		mcu.saveChart("/tmp/charts/cat_detail_upload_rate.pdf");
		...
		DeviationBinnedTallyDisplay mcd = new StackedBinnedTallyDisplay(b);
...
//		mcd.saveChart("/tmp/charts/cat_detail_download_rate.pdf");
		...
		DoubleChart dc = new DoubleChart("Cat__UP/DWN_Details", true, "           Catadupa Traffic Rates", "             final 15 minutes of session", "Download", "Upload", mcu, mcd) ;
		dc.saveChart("/tmp/charts/cat_50k_detailed_upload_download_rates_15min.pdf") ;				
	}
	private void detailed_upload_rates() throws Exception {

		Statistics S0 = Persistent.loadFromXml(cat_filename("A4_M8_250_00_38"));
		
		List<BinnedTally> a = new ArrayList<BinnedTally>();
		add(a, S0.catadupaMsgTraffic.catDaparture_u,"departures");
		add(a, S0.catadupaMsgTraffic.dbRepair_u,"repair");
		add(a, S0.catadupaMsgTraffic.dbEndpoints_u,"endpoints");
		add(a, S0.catadupaMsgTraffic.dbFilters_u, "filters");
		add(a, S0.catadupaMsgTraffic.casting_u, "broadcasting");

		List<BinnedTally> b = new ArrayList<BinnedTally>();
		add(b, S0.catadupaRecentAvgTraffic.catDaparture_d,"departures");
		add(b, S0.catadupaRecentAvgTraffic.dbRepair_d,"repair");
		add(b, S0.catadupaRecentAvgTraffic.dbEndpoints_d,"endpoints");
		add(b, S0.catadupaRecentAvgTraffic.dbFilters_d, "filters");
		add(b, S0.catadupaRecentAvgTraffic.casting_d, "broadcasting");
...
//		mcd.saveChart("/tmp/charts/cat_detail_download_rate.pdf");
		...
		new DoubleChart("Cat__UP_Details", true, "Catadupa Detailed Upload Rates", "entire session", "last 15 minutes", mcu, mcd).saveChart("/tmp/charts/cat_50k_detailed_upload_rates.pdf") ;		
	}
	
	private void detailed_download_rates() throws Exception {

		Statistics S0 = Persistent.loadFromXml(cat_filename("A4_M8_250_00_38"));
		
		List<BinnedTally> a = new ArrayList<BinnedTally>();
		add(a, S0.catadupaMsgTraffic.catDaparture_d,"departures");
		add(a, S0.catadupaMsgTraffic.dbRepair_d,"repair");
		add(a, S0.catadupaMsgTraffic.dbEndpoints_d,"endpoints");
		add(a, S0.catadupaMsgTraffic.dbFilters_d, "filters");
		add(a, S0.catadupaMsgTraffic.casting_d, "broadcasting");
		...
		mcu.chart().chart().getXYPlot().setRangeAxis(new LogarithmicAxis(""));
		commonConfig(mcu, "", "", "byte / s", "session duration (h)");
		mcu.chart().setXRange(false, 10/60.0, 8);
		mcu.chart().setYRange(false, 10, 15000);
		
//		mcu.saveChart("/tmp/charts/cat_detail_download_rate.pdf");

		DeviationBinnedTallyDisplay mcd = new StackedBinnedTallyDisplay(b);
		
		mcd.chart().chart().getXYPlot().setRangeAxis(new LogarithmicAxis(""));
		commonConfig(mcd, "", "", "byte / s", "session duration (h)");
		mcd.chart().setXRange(false, 10/60.0, 8);
		mcd.chart().setYRange(false, 10, 15000);

//		mcd.saveChart("/tmp/charts/cat_detail_download_rate.pdf");
		
		new DoubleChart("Cat_DOWN_Details", true, "Catadupa Detailed Download Rates", "entire session", "last 15 minutes", mcu, mcd).saveChart("/tmp/charts/cat_50k_detailed_download_rates.pdf") ;		
	}

	private void _foo() throws Exception {

		Statistics S0 = Persistent.loadFromXml(cat_filename("A4_M8_250_00_38"));
		Statistics S1 = Persistent.loadFromXml(cat_filename("A2_M4_250_00_76"));
		Statistics S2 = Persistent.loadFromXml(cat_filename("A8_M16_250_00_19"));
		
		List<BinnedTally> a = new ArrayList<BinnedTally>();
		add(a, S0.catadupaMsgTraffic.upload,"(A) 4h - 3.8");
		add(a, S1.catadupaMsgTraffic.upload,"(B) 2h - 7.6");
		add(a, S2.catadupaMsgTraffic.upload,"(C) 8h - 1.9");

		List<BinnedTally> b = new ArrayList<BinnedTally>();
		add(b, S0.catadupaMsgTraffic.download,"(A) 4h - 3.8");
		add(b, S1.catadupaMsgTraffic.download,"(B) 2h - 7.6");
		add(b, S2.catadupaMsgTraffic.download,"(C) 8h - 1.9");
...
//		new DoubleVerticalChart("Cat_Sessions", "Catadupa Traffic Rates", "Upload","Download", mcu, mcd).saveChart("/tmp/charts/cat_50k_session_rates.pdf") ;		
	...
	private void sessionRatios() throws Exception {
		...
		changeNames(SX, "(U) 4h - 3.8 - unbiased");
...
//		tb.add(SX.catadupaRecentAvgTraffic.ratio);
		...
		List<BinnedTally> tu = new ArrayList<BinnedTally>();
		tu.add(SX.catadupaRecentAvgTraffic.ratio);
...
//		new MultiChart("Cat Ratios", false,"Catadupa Upload/Download Ratios", mcd, mcu, xxx) {
//			public void config() {
//				setSubChartTitle(0,"Whole session") ;
//				setSubChartTitle(1,"Last 15 minutes") ;
//				setSubChartTitle(2,"Caca") ;
//			}
//		};
	}

	private void sessionRatiosDeviation() throws Exception {
...
//		mcd.saveChart("/tmp/charts/cat_50k_avg_session_download_rate.pdf");
		...
		new DoubleChart("Cat_DeviationSessions", true, "Catadupa Upload/Download Ratio Fairness", "entire session","last 15 minutes", mcu, mcd).saveChart("/tmp/charts/cat_50k_session_deviation_ratios.pdf") ;		
