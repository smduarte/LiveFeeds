...
import livefeeds.twister7.gui.NodeDisplay;
import livefeeds.twister7.msgs.CatadupaCast;
import livefeeds.twister7.msgs.CatadupaCastPayload;
import livefeeds.twister7.msgs.CatadupaSocketHandler;
import livefeeds.twister7.msgs.CatadupaSocketcReplyHandler;
import livefeeds.twister7.msgs.CatadupaUpdate;
import livefeeds.twister7.msgs.DbRepairReply;
import livefeeds.twister7.msgs.DbRepairRequest;
import livefeeds.twister7.msgs.DbUploadAccept;
...
import livefeeds.twister7.msgs.DbUploadReject;
import livefeeds.twister7.msgs.FailureNotice;
import livefeeds.twister7.msgs.JoinRequest;
import livefeeds.twister7.msgs.JoinRequestAccept;
import livefeeds.twister7.stats.NodeStats;
...
public class CatadupaNode extends AbstractNode implements CatadupaSocketHandler, Displayable, Comparable<CatadupaNode> {
	...
	public String toString() {
		return String.format("%d", index, key);
	}
	...
	public void shutdown() {
		...
		if (state.joined && state.sessionBegin > 0.01*Config.MAX_SESSION_DURATION) {
...
//		double due = 120 * 3600 - currentTime() ; // CAUSE A MASSIVE 40% CRASH AT 120Hours
//		if (due > 0 && (key % 10) <= 4)
//			new Task(this, due) {
//				public void run() {
//					shutdown();
...
//			};
	...
	private void aggregateCatadupaUpdates() {
		...
		hPriority.send(sequencer.endpoint, m, 0, new CatadupaReplyHandler() ) ;
	}
	...
	private void broadcastCatadupaUpdates() {
		...
		CatadupaCastPayload bp = new CatadupaCastPayload(stamp, state.joins, state.rejoins, state.keys, state.departures);
	...
	private double deltaRecentSessionRatio() {
		return (state.stats.catadupaTraffic.recent_upload_rate() - state.stats.catadupaTraffic.recent_download_rate()) / (1 + state.stats.catadupaTraffic.recent_download_rate()) ; 
	}
...
//		System.out.println("--------------->>>>>>" + diffs.first.size() + "/" + diffs.second.size() + "---->" + repairPeriod );
